...
const clipboard = new Clipboard(Deno.build.os);
const fortranData: string = await clipboard.readText();
const juliaData = fortranData
  .split(",")
  .map((a) =>
    a
      .split("&")
  ...
  .flat()
  .map((d: string) => parseFloat(d).toExponential(3));
await clipboard.writeText(`[${juliaData.join(", ")}]`);
